...
student_comparision_sys_bysection <- systems_courses %>%  
  ...
  group_by(academic_year, Semester,CATALOG_NUMBER, CLASS_SECTION) %>% 
...
student_comparision_sys <- student_comparision_sys_bysection %>%
  ...
  filter(Ratio != 0) %>% 
  complete(CATALOG_NUMBER)

ui <- fluidPage(
  
  titlePanel("Class Popularity by Year"),
  wellPanel(
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    d %>% 
      ...
      ggtitle("Course Popularity by Metrics")+
      xlab("Catalog Number")
